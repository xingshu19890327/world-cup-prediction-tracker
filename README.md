...
- 手动点击“更新实际赛果”时，前端会调用 `/api/espn-results`，尝试从 ESPN scoreboard 抓取已完赛比分，并只写入 matchNo >= 16 且尚未填写实际比分的本地比赛。
...
- 只有用户点击“更新实际赛果”按钮时，才会请求一次 ESPN scoreboard。
- 只自动更新 matchNo >= 16 且 `actualScore` 为空的比赛。
- 已有 `actualScore` 的比赛默认不会被覆盖，会计入“已有比分跳过”。
- 如果 ESPN 暂时不可用或尚未返回新的已完赛赛果，用户仍可手动填写实际比分或导入 JSON 作为 fallback。
