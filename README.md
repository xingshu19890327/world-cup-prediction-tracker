...
- 快捷筛选：全部、已完赛、未赛、ChatGPT/Claude 赛果命中、比分命中、双方都错、预测分歧、赔率缺失、高赔率。
...
项目包含安全 migration：旧字段 `wdlPrediction`、`predictedScore1/2/3`、旧赔率字段会迁移到 ChatGPT 对应字段；Claude 字段缺失时保持为空；旧的美东时间字段不会再用于显示或导出。
