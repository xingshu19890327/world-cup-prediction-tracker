...
实际赛果 **不是实时同步**，不会页面加载自动抓取，也没有后台轮询或 `setInterval`。只有点击 **更新实际赛果** 时，前端才会调用 `/api/results` 尝试抓取一次。

当前接口会在没有可靠、高置信数据源时安全失败，并保留已有比分；页面会显示中文失败原因。后续如接入数据源，也只允许通过 `matchNo` 一致，或主队/客队 + 日期时间一致的高置信匹配写入实际比分、实际胜负、完赛状态和最后更新结果时间，不确定匹配不会更新。
